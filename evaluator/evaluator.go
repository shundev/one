...
func evalBooleanInfixExpression(operator string, right, left object.Object) object.Object {
	leftVal := left.(*object.Boolean).Value
	rightVal := right.(*object.Boolean).Value
	switch operator {
	case "==":
		return nativeBool(rightVal == leftVal)
	case "!=":
		return nativeBool(rightVal != leftVal)
	default:
...
func evalStringInfixExpression(operator string, right, left object.Object) object.Object {
	...
	case "+":
		...
		return &object.String{Value: rightVal + leftVal}
	case "*":
		leftVal := left.(*object.Integer).Value
		rightVal := right.(*object.String).Value
		if leftVal < 0 {
			return newError("string multiplier must be zero or a positive integer: %d", leftVal)
		}
		var out bytes.Buffer
		for i := int64(0); i < leftVal; i++ {
			out.WriteString(rightVal)
		}
...
func evalIntegerInfixExpression(operator string, right, left object.Object) object.Object {
	...
	switch operator {
	case "+":
		return &object.Integer{Value: rightVal + leftVal}
	case "-":
		return &object.Integer{Value: rightVal - leftVal}
	case "*":
		return &object.Integer{Value: rightVal * leftVal}
