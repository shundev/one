...
`
	...
	if len(result.Pairs) != len(expected) {
		t.Fatalf(
			"Hash has wrong num of pairs. got=%d",
			len(result.Pairs),
		)
	}
...
func TestBuiltinPush(t *testing.T) {
	...
	testIntegerObject(t, result.Elements[3], 10)
}

func TestBuiltinRest(t *testing.T) {
	input := `rest([1, 2, true, "hello"])`
	evaluated := testEval(input)
	result, ok := evaluated.(*object.Array)
	if !ok {
		t.Fatalf(
	...
	testBooleanObject(t, result.Elements[1], true)
	testStringObject(t, result.Elements[2], "hello")
}
...
let fnD = fn(a) { a * a };
let fnE = fn(b) { b * 2 };
let apply = newApplier(fnD, fnE);
apply(5, 7, fn(c, d) { c - d; });
  `
...
func TestFunctionApplication(t *testing.T) {
	tests := []struct {
		...
		{"let double = fn(x) {return x * 2;}; double(3);", 6},
		{"let add = fn(x,y) {return x + y;}; add(3, 7);", 10},
		{"let add = fn(x,y) {x + y;}; add(3 + 7, add(7, 1));", 18},
		{"fn(x){x;}(5)", 5},
	}
...
`
	...
	for i, tt := range tests {
		...
		if !ok {
			t.Errorf(
				"%d: no error object returned. got=%T(%+v)",
				i,
				evaluated,
				evaluated,
			)
...
func TestIfElseExpressions(t *testing.T) {
	tests := []struct {
		...
		{"if (1 < 2) {40}", 40},
		{"if (1 > 2) {50}", nil},
		{"if (1 > 2) {60} else {70}", 70},
		{"if (1 < 2) {80} else {90}", 80},
	}
